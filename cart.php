                ...
                <article class="listArticlePanier">
                    ...
                    if(isset($_SESSION['cart'])){
                        for($i = 0; $i<count($_SESSION['cart']);$i++){
                            ...
                            $req->execute(['id'=>$id]);
                            $res = $req->fetch();
                            print('<div class="articlePanier">
                                <img class="imagesArticlePanier" src="'.$res['url_images'].'" alt="imageDuJeu">
                                <h4 class="titreArticlePanier">'.$res['nom'].'</h4>
                                <p class="prixArticlePanier">'.$res['prix'].'€</p>
                                <a class="supressionPanier" href="removeToCart.php?id='.$res['id'].'">Supprimer</a>
                                </div>');
                            $prixGlobal += $res['prix'];
                            $nbArticle += 1;
                        }
